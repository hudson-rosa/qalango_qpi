...
@callback(
    Output("dash--tine-chart-tests-duration", "children"),
    Input("dash--project-dropdown", "value"),
)
def update_test_efforts_duration_line_chart(selected_project_id):
    if not selected_project_id:
        return render_message_for_unselected_project()

    aggregated_data = TestEffortsMapper.filter_test_names_and_duration_dictionary(
        selected_project_id
    )
    ...
    line_fig = LineChart(
        **dict(
            [
                (
                    "data_frame",
                    TestEffortsMapper.filter_test_names_and_duration_dictionary(
                        selected_project_id
                    ),
                ),
                ("template", "plotly_dark"),
    ...
    return dcc.Graph(
        ...
        figure=line_fig.create(
            x_axis=Constants.ScenariosDataJSON.SCENARIO_NAME,
            y_axis=Constants.ScenariosDataJSON.TEST_DURATION,
